...
Geometry::Ray convertNormalized2DPointToRay(float normalizedX, float normalizedY)
{
    ...
    Vector nearPointRay = Vector(nearPointWorld[0], nearPointWorld[1], nearPointWorld[2]);
    Vector farPointRay  = Vector(farPointWorld[0],  farPointWorld[1],  farPointWorld[2]);
...
double distanceBetween(Vector point, Geometry::Ray ray)
{
    ...
    // The length of the cross product gives the area of an imaginary parallelogram having the two vectors as sides.
    // A parallelogram can be thought of as consisting of two triangles, so this is the same as twice the area
    // of the triangle defined by the two vectors. http://en.wikipedia.org/wiki/Cross_product#Geometric_meaning
    double areaOfTriangleTimesTwo = (p1ToPoint * p2ToPoint).norm();
    double lengthOfBase           = (*(ray.vector)).norm();
    // The area of a triangle is also equal to (base * height) / 2. In other words, the height is equal
    // to (area * 2) / base. The height of this triangle is the distance from the source to the ray.
