...
Particle* Interaction::pressedParticle;
...
void Interaction::handleTouchDrag(float normalizedX, float normalizedY)
{
    ...
    //if (typeid(Computer::currentComputer) == typeid(ParticleComputer)) {
        if (pressedParticle != nullptr) {
            // Define a plane 1. parallel to default view plane and 2. crossing our Particle
            Vector pressedParticlePosition = Vector(*pressedParticle->r);
            Vector straightVector = Vector(0, 0, 1);
            Geometry::Plane plane = Geometry::Plane(&pressedParticlePosition, &straightVector);
            // Find out where the touched point intersects the aforementioned plane. We'll move the particle along it.
            Vector touchedPoint = intersectionPoint(ray, plane);
            *pressedParticle->r = Vector(touchedPoint.x, touchedPoint.y, pressedParticle->r->z);
